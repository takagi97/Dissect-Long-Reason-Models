...
def compute_grpo_outcome_advantage(token_level_rewards: torch.Tensor,
    ...
    with torch.no_grad():
        ...
        if repetition_penalty_list is None:
            repetition_penalty_list = [False for _ in range(bsz)]
        for i in range(bsz):
        ...
        for i in range(bsz):
            if mask_truncated_samples:
                # It is not a correct sample, and it has reached the maximum length without duplicate penalties
                if not corr_list[i] and eos_mask[i].sum() == response_length and repetition_penalty_list[i]:
                    scores[i] = 0
                else:
...
def compute_policy_loss(old_log_prob, log_prob, advantages, eos_mask, cliprange, loss_remove_token_mean=False):
    ...
    if loss_remove_token_mean is True:
        pg_loss = (torch.max(pg_losses, pg_losses2) * eos_mask).sum() / eos_mask.shape[-1]
...
def compute_token_on_off_policy_loss(
    ...
    """A simple luffy loss adapted from https://github.com/ElliottYan/LUFFY/blob/768159222d78c5c1f54f413c157c5cee5c4e5537/luffy/verl/verl/mix_src/mix_core_alg.py#L69
    ...
    """
    # off-policy loss
    prefix_mask = prefix_mask.unsqueeze(1).repeat(1, log_prob.size(1))
