...
cp -r scripts/train/8k_pos.sh deepscaler/checkpoints/deepscaler/$MODEL_NAME
cd deepscaler

# We mask out all negative samples and update the policy model using only positive samples by setting cut_down_negative_samples=True.
# We set cut_down_samples_reserve_ratio=0.0 and cut_down_samples_reserve_min_num=0.0 to ensure that no negative examples are retained.
python3 -m verl.trainer.main_ppo \
    algorithm.adv_estimator=grpo \
    algorithm.cut_down_negative_samples=True \
